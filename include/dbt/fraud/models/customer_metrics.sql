WITH transaction_sumary AS (
    SELECT
        ct.user_id,
        COUNT(ct.transaction_id) AS total_transactions,
        SUM(CASE WHEN lt.is_fraud THEN 1 ELSE 0 END) AS fraudulent_transactions,
        SUM (CASE WHEN NOT lt.is_fraud THEN 1 ELSE 0 END) AS non_fraudulent_transactions
